...
from pfio.v2._profiler import record
...
class MultiprocessFileCache(cache.Cache):
    ...
    def get(self, i):
        with record("pfio.cache.multiprocess:get", trace=self.trace):
            if self.closed:
                return
            data = self._get(i)
    ...
    def _get(self, i):
        ...
        with record(f"pfio.cache.multiprocess:get:lock-{self.cache_fd}",
                    trace=self.trace):
    ...
    def put(self, i, data):
        with record("pfio.cache.multiprocess:put", trace=self.trace):
    ...
    def _put(self, i, data):
        ...
        with record(f"pfio.cache.multiprocess:put:lock-{self.cache_fd}",
                    trace=self.trace):
